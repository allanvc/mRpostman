...
functionality of the IMAP4rev1 protocol (RFC 3501), allowing you to
perform virtually all e-mail operations from within R. The aim of this
package is to pave the way for email data analysis in R. To do so,
`mRpostman` makes extensive use of the {curl} package and the libcurl C
library.
...
which every compliant server must implement — plus a set of **optional
extensions**, each advertised by the server in its `CAPABILITY`
response. `mRpostman` covers both. For the extension-based methods,
`mRpostman` checks the server’s advertised capabilities and, if the
required one is missing, raises an informative error instead of letting
the server reply with a cryptic `BAD Unknown command`. You can inspect
what your server supports with `list_server_capabilities()`.
...
365). Every registered IMAP4rev1 extension in common use is now
implemented; the raw-socket methods (`idle()`, `notify()`,
`append_msgs()`, `append_catenate()`, `fetch_binary()`) need an
`imaps://` URL for TLS, since STARTTLS is not available on that
connection.
