...
#'     (Babcock, 2016).
#'
#' @references Resnick, P., Ed., "Internet Message Format", RFC 5322,
...
#' # search 1
#' result1 <- imapconf %>%
#'     selectMailbox(mbox = "[Gmail]/Sent") %>%
#'     searchBefore(date_char = "17-Apr-2012")
#'
#' # search 2 (brazilian portuguese example -- mailboxes with compound names)
#' result2 <- imapconf %>%
#'     selectMailbox(mbox = "[Gmail]/Com estrela") %>%
#'     searchBefore(date_char = "17-Dec-2018", flag = "ANSWERED")
...
#' @export
#'
searchBefore <- function(imapconf, date_char, negate = FALSE, by = "MSN",
  ...
  h <- define_searchrequest_date(operation = "BEFORE",
