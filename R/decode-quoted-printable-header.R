...
#'   RFC 2047 encoded-word. When supplied, it is used directly (via
#'   \code{\link{apply_charset}}) instead of the legacy heuristic detection.
#' @noRd
decode_quoted_printable_header <- function (qp_encoded, charset = NULL) {
  ...
  if (!is.null(charset)) {
    # RFC 2047 declares the charset in the encoded-word: trust it (any iconv-
    # supported charset works, e.g. Windows-1251, ISO-8859-2, Big5, Shift_JIS).
    decoded_string <- apply_charset(decoded_string, charset)

  } else {
    # Legacy heuristic fallback, used for "loose" quoted-printable strings that
    # do not carry a declared charset (e.g. "Estat=EDstica").
    tryCatch({
