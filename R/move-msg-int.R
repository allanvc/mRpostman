#' Move message(s) between the selected folder and another one (INTERNAL HELPER)
...
#'   Default is \code{1}.
#' @noRd
move_msg_int <- function(self, msg_id, use_uid, to_folder, reselect, mute, retries) {

  check_args(msg_id = msg_id, use_uid = use_uid, folder = to_folder,
  ...
  if (isTRUE(use_uid)) {
    ...
    customrequest <- paste0("UID MOVE ", msg_string, " ", to_folder2)

  } else {

    customrequest <- paste0("MOVE ", msg_string, " ", to_folder2)
  ...
  if (!mute) {
    cat(paste0("\n::mRpostman: message(s) moved")) # v0.3.2
