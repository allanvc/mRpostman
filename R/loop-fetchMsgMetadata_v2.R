...
#' @keywords internal
#'
loop_fetchMsgMetadata <- function(new_imapconf, msg_id, by, metadata,
  ...
  for(id in msg_id){
    ...
    curl::handle_setopt(
      handle = h,
      customrequest = paste0(by_string, "FETCH ", id, " (", metadata, ")"))
      ...
      while(is.null(response) && count_retries < retries){
        ...
        if(!is.null(response)){
          ...
          if(isTRUE(write_to_disk)){
            ...
            complete_path <- paste0("./", mbox_clean)
            dir.create(path = complete_path, showWarnings = FALSE,
                       recursive = TRUE)
