#' @description Subscribe to a mail folder (INTERNAL HELPER)
#' @param name A string containing the name of the mail folder to subscribe to.
...
#'   Default is \code{1}.
#' @noRd
subscribe_folder_int <- function(self, name, mute, retries) {
  ...
  response <- imap_exec(self, customrequest = paste0('SUBSCRIBE ', name2),
                        retries = retries)$response

  if (!mute) {


    cat(paste0("\n::mRpostman: folder ", '"', name, '"', " subscribed.\n"))
