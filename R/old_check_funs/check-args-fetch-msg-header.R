#' @title Header Fetch Arguments Check
#'
...
#' @param fields A character vector specifying which field(s) to fetch from the
#'     message header. See \link{section_or_field_options}
#' @param negate_fields If \code{TRUE}, negates the search and seeks for
...
#' @keywords internal
#'
check_args_fetch_msg_header <- function(msg_id, by, fields, negate_fields, peek,
  ...
  assertthat::assert_that(
    ...
    msg='"fields" must be NULL or a character vector. See section_fields_optons().')
