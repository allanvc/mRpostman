...
#' @param field A mandatory \code{character} string specifying in which
#'     Header Field to search for the provided string.
#'     For some available options, see \code{\link{field_options}}.
...
#' @keywords internal
#'
check_args_search_string_in_header <- function(string, field, negate, by, esearch,
                                               retries) {
  ...
  assertthat::assert_that(
    is.character(field),
    msg='"field" argument must be a character. See field_options().')
