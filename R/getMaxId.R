...
#'
#' @inheritParams check_args_getMaxMinId
#'
#' @return A numeric \code{vector} of length \code{1} indicating the minimum id.
#'
#' @note \code{getMaxId} uses \code{ESEARCH} extension. Check if your server
#'     supports \code{ESEARCH} capability with \link{listServerCapabilities}.
#'
...
#'
#' results <- imapconf %>%
#'     selectMailbox(mbox = "[Gmail]/Trash") %>%
#'     getMaxId(by = "UID", flag = "SEEN")
#'
#' }
#'
#' @export
#'
getMaxId <- function(imapconf, by = "MSN", flag, retries = 2){
