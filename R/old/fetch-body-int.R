...
#' @keywords internal
#'
fetch_body_int <- function(self, msg, use_uid, peek, partial, write_to_disk, silent,
                           retries) {

  # print(msg)
  # we accept silent here only to do check
  #check
  check_argg(msg = msg, use_uid = use_uid, peek = peek, partial = partial,
             write_to_disk = write_to_disk, silent = silent, retries = retries)
  ...
  msg_list <- loop_fetch_body(self, msg, use_uid, peek, partial, write_to_disk, retries,
                              handle = h)
