...
flag_options <- function() {
  ...
  flag <- c("SEEN", "ANSWERED", "FLAGGED", "DELETED", "DRAFT", "RECENT", "NEW")

  antonym_flag <- c("UNSEEN", "UNANSWERED", "UNFLAGGED", "UNDELETED", "UNDRAFT", NA,
                        "OLD")
  ...
  flags_df <- data.frame(flag, description, antonym_flag)
