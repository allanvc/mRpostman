#' @title Add Flag(s) To Messages
#'
#' @description Add one or more flags to one or more messages.
#'
#' @inheritParams check_args_add_replace_flags
#'
#' @note \code{\link{add_flags}}, \code{\link{remove_flags}} and
#'     \code{\link{replace_flags}}, accepts not only system flags but also keywords
#'     (custom flags).
#'
...
#'
#' @return An (invisible) \code{vector} containing the the previously inputed
#'     message ids (argument \code{msg_id}).
#'
...
#'      add_flags(flags_to_set = "\\RECENT")
#'
#' }
#'
#' @export
#'
add_flags <- function(msg_id, by = "MSN", flags_to_set, retries = 2) {
