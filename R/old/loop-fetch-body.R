...
#' @keywords internal
#'
loop_fetch_body <- function(self, msg, use_uid, peek, partial, write_to_disk,
                            retries, handle) {
  ...
  for (id in msg) {
    ...
    tryCatch({
      curl::handle_setopt(
        handle = h,
        customrequest = paste0(use_uid_string, "FETCH ", id, body_string, partial_string))
    }, error = function(e){
    ...
    if (!is.null(response)) {
      ...
      if (!exists("response")) {
        stop('Fetch error: the server returned an error. Try to increase "timeout_ms".') # v0.0.9.0
      }
