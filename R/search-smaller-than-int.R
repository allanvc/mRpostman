#' Search by size (SMALLER) (INTERNAL HELPER)
...
#'   Default is \code{1}.
#' @noRd
search_smaller_than_int <- function(self, size, negate, use_uid, flag, esearch,
                                    retries) {

  check_args(size = size, negate = negate, use_uid = use_uid, flag = flag,
  ...
  define_out <- define_searchrequest_size(operation = "SMALLER",
