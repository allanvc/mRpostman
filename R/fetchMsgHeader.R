...
#'
#' @inheritParams check_args_fetchMsgHeader
#'
...
#'     fetchMsgHeader(imapconf = imapconf, msg_id = msg_id, fields = c("From", "To"))
#'
#' }
#'
#' @export
#'
fetchMsgHeader <- function(imapconf, msg_id, by = "MSN", fields = NULL,
                           negate_fields = FALSE, peek = TRUE,
                           partial = NULL, write_to_file = FALSE,
                           keep_in_mem = TRUE, retries = 2){

  # checks
  check_args_fetchMsgHeader(imapconf, msg_id, by, fields, negate_fields, peek,
  ...
  msg_list <- loop_fetchMsgHeader(new_imapconf, msg_id, by, fields,
