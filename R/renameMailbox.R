...
#'     \code{configureIMAP} and with a \code{mbox} item added after a
#'     mailbox selection with \link{selectMailbox}.
#' @param new_name A string containing the new name to be set.
#' @param reselect_mbox If \code{TRUE}, calls \code{selectMailbox(mbox = to_mbox)}
...
#' @export
#'
renameMailbox <- function(imapconf, new_name, reselect_mbox = TRUE, retries = 2){
  ...
  if(isTRUE(reselect_mbox)){
    ...
    new_imapconf <- selectMailbox(imapconf = new_imapconf,
