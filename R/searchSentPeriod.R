...
#'
#' @return Depending on the \code{return_imapconf} parameter, it can be a
...
#'     only message ids.
#'
#' @note \link{searchBefore}, \link{searchSince}, link{searchOn}, and
#'     \link{searchPeriod} use internal date,
#'     which reflects the moment when the message was received.
#'     \link{searchSentBefore}, \link{searchSentSince}, link{searchSentOn}, and
#'     \link{searchSentPeriod} use RFC-2822 date header (origination date), which
...
#'     searchPeriod(since_date_char = "17-Apr-2012",
...
#' @export
#'
searchSentPeriod <- function(imapconf, since_date_char, before_date_char,
                             negate = FALSE, by = "MSN", flag = NULL,
                             esearch = FALSE, return_imapconf = TRUE,
                             retries = 2){

  check_args_searchPeriod(imapconf, since_date_char, before_date_char, negate,
                          by, flag, esearch, return_imapconf, retries)
  ...
  h <- define_searchrequest_period(
    operation1 = "SENTSINCE",
    since_date_char = since_date_char,
    operation2 = "SENTBEFORE",
  ...
  if(!is.null(response)){
    ...
    selectMailbox(imapconf = new_imapconf, mbox = new_imapconf$mbox)
  ...
  # fix stripping
  response <- fix_searchStripping(response)
