...
#'
#' @inheritParams check_args_searchWithin
#'
...
#'     only message ids.
#'
#' @note Some SMTP servers, such as Gmail and AOL, may not support WITHIN SEARCH
#'     EXTENSION.
#'
...
#' # search
#' results <- imapconf %>%
#'     selectMailbox(mbox = "Smiles GOL") %>%
#'     searchYoungerThan(seconds = 3600)
#'
#' }
#'
#' @export
#'
searchYoungerThan <- function(imapconf, seconds, negate = FALSE, by = "MSN",
  ...
  h <- define_searchrequest_within(operation = "YOUNGER",
  ...
  if(!is.null(response)){
    ...
    selectMailbox(imapconf = new_imapconf,
