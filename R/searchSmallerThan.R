#' @title Search By Syze
#'
#' @description Searches for messages by bytes size.
#'
...
#'     searchSmallerThan(size = 512000) # smaller than 512KB
#'
#' }
#'
#' @export
#'
searchSmallerThan <- function(imapconf, size, negate = FALSE, by = "MSN",
                              flag = NULL, esearch = FALSE, return_imapconf = TRUE,
                              retries = 2){

  check_args_searchSize(imapconf, size, negate, by, flag, esearch, return_imapconf,
                        retries)
  ...
  h <- define_searchrequest_size(operation = "SMALLER",
