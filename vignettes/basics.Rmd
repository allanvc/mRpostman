...
**mRpostman** makes extensive use of 'curl' and 'libcurl' capabilities, providing tools for searching and fetching emails, mailbox management, attachment extraction, and several other IMAP functionalities.
...
**IMPORTANT**: In version `0.9.0.0`, `mRpostman` went trough thorough changes, including ones that have no backward compatibility with versions `<= 0.3.2`. A detailed vignette on how to migrate your mRpostman's deprecated code to the new one is availabe at: ["Migrating code to mRpostman's R6 sintaxe"](https://allanvc.github.io/mRpostman/articles/code_migration.html) for more information.

## Authentication

There are two ways of connecting to your IMAP server: using plain or OAuth2.0 authentication. In this vignette, we only describe the plain authentication process. If you want to use OAuth2.0 authentication, please refer to the _"Using IMAP OAuth2.0 authentication in mRpostman"_ vignette.

### Allowing Less Secure Apps Access

When using plain authentication, most of mail providers will require the user to enable the **less secure apps** access. Once it is done, you will be able to access you mailbox using a "third part app" like `mRpostman`.
...
The package is now implemented under a OO framework, using an R6 class. The main functionalities of `mRpostman` are implemented as methods of the R6 class called `ImapCon`. These methods are described below:
...
After enabling (if needed) "Non secure apps access" in your Mail provider or obtaining an OAuth2.0 token, you have to configure an IMAP connection:
...
con <- configure_imap(url="imaps://outlook.office365.com",
                      username="your_user@your_company.com",
...
# Yahoo Mail
# con <- configure_imap(url="imaps://imap.mail.yahoo.com/",
#                       username="your_user",
#                       password=rstudioapi::askForPassword()
#                      )

# AOL Mail
# con <- configure_imap(url="imaps://export.imap.aol.com/",
#                       username="your_user",
#                       password=rstudioapi::askForPassword()
#                       )

# Yandex Mail
# con <- configure_imap(url="imaps://imap.yandex.com",
#                       username="your_user",
#                       password=rstudioapi::askForPassword()
#                       )

# you can try any other Mail provider
...
Since version `0.9.0.0`, this package provides more flexibility to the user in the sense that you can modify the connection parameters for specific commands or parts of a script, using the `reset_*()` methods. This prevents user from having to call `configure_imap()` multiple times during a session or in an script. It is particularly useful when the user is going to perform some fetch operation, for example. In this case, it is recommended to increase the `timeout_ms` and set `verbose = FALSE`.
...
Once the connection is configured, it is important to know which capabilities your IMAP provider offers to users. This impacts on which type of operations you are allowed to perform. For example, if your server has the `WITHIN` extension you can use the WITHIN search methods `search_younger_than()` and `search_older_than()`; if the server has the `ESEARCH` capability, besides being allowed to use the `esearch_*()` methods, you can optimize all your search functions with the `esearch = TRUE` parameter; if you see the `MOVE` capability, then you are able to use the `move_msg()` method. Therefore, in order to know all your server capabilities, you can use `list_server_capabliltites`.
...
With the exception of `examine_folder()` and `rename_folder()`, from now on, you will have to select a folder to issue further commands.
...
Flags work like tags or labels attached to messages. You can check the options which flags are available, if you are allowed to set custom flags of your own in a mail folder with `list_flags()` after a mail folder is selected.
...
All search methods will return a numeric vector containing the results from search. This allows users to chain fetch operations together with search one. You can also **NEGATE** all search expressions by setting `negate = TRUE`.

If your server supports **ESEARCH**, we recommend you to use it. It will prevent your results from being
truncated when there is too many messages ids and you didn't set a high `buffersize` in `confiure_imap()`^[This is a know bug of the libcurl library. Please, refer to this [LINK](https://curl.haxx.se/docs/knownbugs.html#IMAP_SEARCH_ALL_truncated_respon)]. With "ESEARCH", the results will be condensed to groups of sequence similar to what \R does. For instance, if your search returns 10000 results, it is better having condensed groups such as `1:10, 12, 23:27, ...` instead of a sequence of `1, 2, 3, 4, 5, 6, ..., 10, 12, 23, 24, 25, ...`.   If you can't use ESEARCH, or if your results are being truncated even with ESEARCH, you can try to increase your buffersize in `configure_imap()` in order to avoid this.
...
delivery system" (Resnick, 2008). Dates in both methods must be the same most of time. The difference may occur when you copied or moved some messages between folders. In this case, the RFC-2822 date header of the copied/moved messages in the destination folder, will point out to the date of the copy.  Another difference is that searching by the internal date will probably be faster because this information is kept in a database outside the message.
...
You can use the UID (unique identifier) instead of the message sequence number ^[A message sequence number is a message's relative position to the oldest message in a mail folder. It may change after deleting or moving messages. If a message is deleted, sequence numbers are reordered to fill the gap. If `use_uid = TRUE`, the command will be performed using the "UID" or unique identifier, and results are presented as such. UIDs are always the same during the life cycle of a message in a mail folder.], and one or more flags as an additional filter to your search. Actually, you can use this in almost every search method of this package.
...
Remember that, if your IMAP server has the ESEARCH capability, you can use it. GMail is one of the providers that allows it.
...
You can also **NEGATE** the statement and search for messages (or specific part of a message) not containing that string, and add additional flag filters as well. 
...
Severs with support to the "WITHIN" EXTENSION enable searching for messages within a span of time, i.e. younger than "x" seconds, or older than "x" seconds. This capability is really rare of find in IMAP servers, but `mRpostman` has two methods implemented for coping with this capability if it is available.
...
**Example 2**: Searching (usinbg UID) for messages received from "@k-state.edu" **OR** "@ksu.edu".
...
You can fetch the full content of messages, or their parts, such as the header, text, or specific metadat fields. Besides this, you can also fetch a message attachments list or the attachment files themselves, downloading them to the disk.
...
In the main fetch methods (those that are not related to attachment fetching), you can choose to write fetch results to disk (working directory) using `write_to_disk = TRUE`. If you opt to do so, **mRpostman** saves the fetch content to a `.txt` file in the following folder structure: `working directory > imap.server.url > mail folder name`. The text files will be named after the id of the fetched message. If the operation was executed using the UID, the "UID" prefix are added to the file names.

**IMPORTANT**:

1. If the fetch operation is to be chained after a search, the `use_uid` arguments in the two operations have to be the same. Otherwise, an error will occur or the the fetch will be performed on wrong messages' ids.
...
![](figures/basics/fetch_body.png){width=90%}
...
Note that, in the example above, we are saving the results to the `out` object in our R session. Also note that we are setting `use_uid = TRUE` in both search and fetch requests.
...
The are two more fetch methods, but they are going to be presented in the next session, since they are relate to a very special fetch operation.
...
Let's take a look in the first two attachments functionalities available in `mRpostman`.
...
`get_attachments()` will try to decode base64 text inside your message to the appropriate file extension. This will create a folder with the inside the structure `wd > imap.server.name > mail folder > msg_id`. The attachment(s) will be inside their respective message's folder.
...
![](figures/basics/get-fetch_attachments1.png){width=90%}
![](figures/basics/get-fetch_attachments2.png){width=90%}

### 7.2) Optimized attachment operations

As we stated before, the following two methods do not dependent on a previous fetch step, because they execute a fetching operation. Hence, they consist of an optimized alternative for attachments listing and downloading.
...
This operation depends on the ESEARCH capability. It will retrieve the **minimum** message id containing an specific flag(s) in the selected mail folder.
...
This operation also depends on the ESEARCH capability. It will retrieve the **maximum** message id containing an specific flag(s) in the selected mail folder.
...
This operation also depends on the ESEARCH capability. It will retrieve the **number** of messages with an specific flag(s) in the selected mail folder.
...
This method marks one or more messages with the "\\Deleted" system flag. Some servers automatically delete messages marked with this flag, and others require the `EXPUNGE` command in order to definitely remove the e-mail.
...
Expunges message(s) marked with the "DELETED" flag in a mailbox or an specific message using the `msg_uid` argument. Note the this require the "UID" (unique id) and not sequence numbers.
...
con$delete_msg(msg_id = 71171, use_uid = "UID") %>%
  expunge()
...
Note that now we have the `flags_to_UNset` argument.
...
Crispin, M. (2003), *INTERNET MESSAGE ACCESS PROTOCOL - VERSION 4rev1*, RFC 3501,DOI: 10.17487/RFC3501, March 2003, https://tools.ietf.org/html/rfc3501.
...
Freed, N. and Borenstein, N. (1996), *Multipurpose Internet Mail Extensions (MIME) Part Two: Media Types*, RFC 2046, DOI: 10.17487/RFC2046, November 1996, https://tools.ietf.org/html/rfc2046.

Resnick, P. (2001), *Internet Message Format*, RFC 2822, DOI: 10.17487/RFC2822, April 2001, https://tools.ietf.org/html/rfc2822.

Resnick, P. (2008), *Internet Message Format*, RFC 5322, DOI: 10.17487/RFC5322, October 2008, https://tools.ietf.org/html/rfc5322.
