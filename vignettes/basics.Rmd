...
such as listing mailboxes, searching and fetching messages in a tidy way.
...
"WITHIN" extension, you can use `searchYoungerThan()` and `searchOlderThan()` functions.
...
You will need the IMAP configuration object and the message's ids.
...
stripped. With "ESEARCH", the results will be condensed to groups of sequence similar to what R does. For instance, if your search returns 1000 results, it is better having condensed groups such as `1:10, 12, 23:27, ...` instead of a sequence of `1, 2, 3, 4, 5, 6, ..., 10, 12, 23, 24, 25, ...`.   If you can't use ESEARCH, or if your results are being stripped even with ESEARCH, you can try to increase your buffersize in `configureIMAP()` in order to avoid stripping. This is related to curl's buffersize.
...
results <- imapconf %>%
  selectMailbox(mbox = "CRAN messages2") %>%
  searchString(section_or_field = "TO", string = "hadley@rstudio.com",
...
results <- imapconf %>%
  selectMailbox(mbox = "INBOX") %>%
  searchYoungerThan(seconds = 3600) # msgs received in less than one hour (3600 sec)
...
results <- imapconf %>%
  selectMailbox(mbox = "INBOX") %>%
  searchOlderThan(seconds = 3600) # msgs received in not less than one hour (3600 sec)
...
You cannot add a flag that is the antonym of an already existent flag in the message, e.g. cannot add "UNSEEN" flag when a message already has flag "SEEN". Use `replaceFlags()` instead.
...
`moveMsg()` uses IMAP "MOVE" EXTENSION. Check if your server supports "MOVE" capability with `listServerCapabilities()`.
...
If your server does not provide "MOVE" capability, the same result can be achieved with a combination of `addFlags()` and `expunge()`:
