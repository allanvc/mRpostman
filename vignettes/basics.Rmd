...
In both examples, the whole message body(ies) or text will be saved to the disk, but a copy will be kept in the R session. This copy is passed forward to `list_attachments()`, which, by its turn, finds and extracts the filenames and the type of the "Content-Disposition" for the part where the attachment was found.

#### 7.1.2) Extracting Attachments

`get_attachments()` will try to decode base64 text inside your message to the appropriate file extension. This will create a folder inside the structure `working directory > username > mail folder > msg_id`. The attachment(s) will be inside their respective message folder.
...
As we stated before, the following two methods are not contingent on a previous fetch step because they execute their own fetching operation. Hence, they consist of an alternative for attachments listing and downloading.
...
## 8.5) The query language (2.3.0)

Since 2.3.0 a search can be written as an ordinary R expression. `query()` captures the expression unevaluated, translates it into the same RFC 3501 search string through the pure function `translate_query()`, and executes it exactly like `search()`:
...
The criterion constructors of the previous section also gained the native operators: `string("budget", where = "SUBJECT") & !flag("SEEN")` is equivalent to `AND(string("budget", where = "SUBJECT"), flag("SEEN", negate = TRUE))`.

## 9) Protocol extensions (since 1.5.0)
...
### 9.1) UIDPLUS: UIDs of appended and copied messages
...
ids <- con$copy_msg(msg_id = c(12, 15), use_uid = TRUE, to_folder = "Archive",
                    reselect = FALSE)
attr(ids, "copyuid")
```

### 9.2) Folder listings in one round trip
...
### 9.3) Saved search results (SEARCHRES)
...
### 9.4) Server-side previews, saved dates, and change tracking
...
### 9.5) Sorting with server-side aggregates (ESORT)
...
### 9.6) Access control lists and quota limits
...
con$get_acl(name = "Shared")
```

## 10) A second connection for IDLE and MULTIAPPEND (since 2.0.0)
...
The second connection needs the credentials again: they are kept in a private field of the connection object (never printed, not part of `con_params`) and cleared by `disconnect()`. For TLS, the URL must be `imaps://` (STARTTLS is not available on the raw socket).
