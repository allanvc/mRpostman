...
Since 2.3.0 a search can be written as an ordinary R expression. `query()` captures the expression unevaluated, translates it into the same RFC 3501 search string through the pure function `imap_query()`, and executes it exactly like `search()`:
