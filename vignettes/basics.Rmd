...
`mRpostman` provides tools for searching and fetching emails, mailbox management, attachment extraction, and several other IMAP functionalities. This package makes extensive use {curl} to implement an easy-to-use IMAP client for R, paving the way for users to perform data analysis on email data.
...
There are two ways of connecting to your IMAP server: using plain or OAuth2.0 authentication. In this vignette, we only describe the plain authentication process. If you want to use OAuth2.0 authentication, please read the _"Using IMAP OAuth2.0 authentication in mRpostman"_ vignette.
