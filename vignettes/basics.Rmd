...
con$reset_timeout_ms(to = 30000) # ... to 30 secs

# and supposing that you had verbose = TRUE before
con$reset_verbose(to = FALSE)
...
`get_attachments()` will try to decode base64 text inside your message to the appropriate file extension. This will create a folder with the inside the structure `working directory > imap.server.name > mail folder > msg_id`. The attachment(s) will be inside their respective message folder.
