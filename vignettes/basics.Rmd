...
`get_acl()`, `set_acl()`, `delete_acl()`, `list_rights()`, and `my_rights()` implement the `ACL` extension (RFC 4314); `set_quota()` implements `SETQUOTA` (RFC 2087), which most servers restrict to administrators. `enable()` (RFC 5161) enables extensions that require it.
