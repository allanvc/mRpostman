---
title: "Migrating code to mRpostman's R6 sintaxe"
output: 
...
This vignette aims to provide an easy source of code migration examples so that old `mRpostman` users can transition between the old version and the new format which implements an OO approach trough the use of R6 class and methods.
...
## 1) Configurig the IMAP connection
...
Another useful options are: `timeout_ms`, `verbose = TRUE`, and `buffersize`. Further 'curl' options related to IMAP functionalities can be passed to `configure_imap()`, depending on the libcurl version. See `curl::curl_options()`. In this new version there is an important curl parameter exposed, `xoauth2_bearer`, which enables OAuth2.0 authentication. Fore more details, check the _"Using IMAP OAuth2.0 authentication in mRpostman"_ vignette.
...
In the new format, the previous selected folder is kept during the session unless you execute a new selection.
...
All search functions now return only the message's ids.
...
Special attention should be paid to the `search_string` and `search_flag` methods due to changes in the arguments names and order.
...
Now you can check which flags are available in a mail folder with the `list_flags()` method. In addition, you can pass multiple flags as search criterion.
...
Notice that `list_attachment()` is an independent function. It is not a method of the `ImapCon` class that depends on the connection object.
