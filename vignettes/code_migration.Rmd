...
Another useful options are: `timeout_ms`, `verbose = TRUE`, and `buffersize`. Further 'curl' options related to IMAP functionalities can be passed to `configure_imap()`, depending on the libcurl version installed on the user's machine. See `curl::curl_options()`. In this new version there is an important curl parameter exposed, `xoauth2_bearer`, which enables OAuth2.0 authentication. Fore more details, check the _"Using IMAP OAuth2.0 authentication in mRpostman"_ vignette.
