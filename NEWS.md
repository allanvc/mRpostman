...
- **Verbatim criteria with `imap_raw()`**: raw protocol fragments with no dedicated field — vendor extensions such as Gmail's `X-GM-RAW`, sequence sets like `UID 100:200`, `FUZZY` matches — are wrapped as first-class criteria and compose with the query language and the criterion constructors, e.g. `con$query(imap_raw('X-GM-RAW "has:attachment"') & flag != "SEEN")`. The query language thereby covers everything a raw custom `search()` can express.
