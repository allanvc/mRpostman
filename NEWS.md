...
- **`print(con)`** now shows a compact summary (server, user, TLS, selected folder) instead of dumping the whole R6 object, and never prints credentials.
- **`con$has_capability("ESEARCH")`**: a direct predicate over the advertised capabilities.
- **`decode_mime_text()` exported**: the body-text counterpart of `decode_mime_header()`.
- `metadata_options()` now lists the capability-gated extension attributes (`PREVIEW`, `SAVEDATE`, `MODSEQ`) that `fetch_metadata()` accepts.

- **The query language**: new `ImapCon$query()` searches with an ordinary R expression, e.g. `con$query((subject == "budget" | "budget 3") & flag != "SEEN" & size > 5e6)`. Fields: `subject`, `from`, `to`, `cc`, `bcc`, `body`, `text` (`==` means contains), `flag` (system flags and custom keywords, `!=` maps to the `UN*` forms), `size` in bytes and `age` in seconds (`>`, `>=`, `<`, `<=`), the date fields `sent`, `date`, and `saved` (`>=`, `>`, `<`, `<=`, `==`, exact at day granularity), and `header("Name")`. Expressions combine with `&`, `|`, `!`, `%in%` (one field, several values), and parentheses, with R's own precedence; variables and criterion constructors are evaluated in the caller's environment, and a bare string next to `|` inherits the field of the preceding comparison. The translation is a pure internal function, exercised offline by the test suite against expected protocol strings.
- **Verbatim criteria with `verbatim()`**: raw protocol fragments with no dedicated field — vendor extensions such as Gmail's `X-GM-RAW`, sequence sets like `UID 100:200`, `FUZZY` matches — are wrapped as first-class criteria and compose with the query language and the criterion constructors, e.g. `con$query(verbatim('X-GM-RAW "has:attachment"') & flag != "SEEN")`. The query language thereby covers everything a raw custom `search()` can express.
- **Attachment extraction rebuilt on a real MIME parser**: `get_attachments()` and `list_attachments()` now walk the multipart tree by boundary (recursively) instead of scraping the raw text with regular expressions, and `fetch_attachments()` is guided by the `BODYSTRUCTURE` the server reports, fetching each attachment as its exact MIME part. This fixes payloads corrupted by leftover boundary or header fragments, attachments nested in `multipart/alternative`/`related` structures, quoted-printable attachments (previously skipped), filenames in RFC 2231 form (`filename*=UTF-8''...`), unquoted filenames, filenames containing `--`, and a stray NUL byte appended by `as_is = TRUE`. Payloads are verified byte-identically against the server-side decode (`fetch_binary()`) in the test suite.
